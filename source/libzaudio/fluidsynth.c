    ...
    if ((song = SDL_malloc(sizeof(FluidSynthMidiSong)))) {
        ...
        if (SDL_BuildAudioCVT(&song->convert, AUDIO_S16, 2, freq, format, channels, freq) >= 0) {
            if ((settings = fluidsynth.new_fluid_settings())) {
                ...
                /* add by Zgames */
                /* 设置synth.audio-channels后，声音正常了。*/
                fluidsynth.fluid_settings_setint(settings, "synth.midi-channels", 128);
                fluidsynth.fluid_settings_setint(settings, "synth.audio-channels", channels);
                /* end Zgames */
